<template>
  <div>
        <div style="width:100%;clear:both;text-align:left">
            <el-input v-model="searchReportname" placeholder="输入功能名称" style="width:170px;"> </el-input> 
            <el-button type="primary" icon="el-icon-search"  @click="search">搜索</el-button>
            <el-button type="primary" @click="opendialog(1,null)" icon="el-icon-edit">添加功能</el-button>
        ...
        <el-table class="elTable" highlight-current-row  @current-change="handleCurrentChangeTable" ref="table" :data="maindata" stripe :style="`height:${clientHeight}px;width:98%;margin-top:10px;`">
             <el-table-column :show-overflow-tooltip="true" width="65px;" prop="Id" label="ID" ></el-table-column>
          <el-table-column :show-overflow-tooltip="true" width="155px;" prop="ReportName" label="功能名称" ></el-table-column>
          <el-table-column :show-overflow-tooltip="true" width="155px;" prop="MenuName"  label="所属菜单"></el-table-column>
          <el-table-column :show-overflow-tooltip="true" width="105px;" prop="ReportType" label="展示方式"></el-table-column>
          <el-table-column width="55px;"  prop="StatusName" label="状态"></el-table-column>
          <el-table-column :show-overflow-tooltip="true" width="95px;" prop="DataAccess" label="接入方式"></el-table-column>
        ...
        <el-dialog :top="dialogtop" :title="`${currentOpName}`" width="80%" :visible.sync="dialogFormVisible">
          <el-form :model="form" ref="objectForm"  :rules="rules">
                <el-form-item label="功能名称" :label-width="formLabelWidth" prop="reportName" >
                       <el-input :disabled="detailFlag"   :style="`width:${formValueWidth}`" placeholder="请输入功能名称"  v-model="form.reportName" autocomplete="off" clearable></el-input>
                ...
                <el-form-item label="功能脚本" :label-width="formLabelWidth" prop="dataScript" ref="Sys_ActualStartTime">
                     <el-input :disabled="detailFlag" type="textarea" :rows="5" placeholder="请输入SQL功能语句" v-model="form.dataScript"></el-input>
                </el-form-item>
                <!-- <el-form-item label="功能参数" :label-width="formLabelWidth" prop="param">
                ...
                <el-form-item  label="排序语句" :label-width="formLabelWidth" prop="Mat_DefId">
                   ...
                   <el-button @click="createReport" :disabled="detailFlag" type="primary">配置功能</el-button>
                </el-form-item>
...
export default {
  ...
  methods: {
    ...
    loadMemu(){
      this.$http({methods: "get",url:this.URL+"/api/report/menu/0"})
        .then(resp => {
    ...
    createReport(){
      ...
      this.form.param=[];
         arrayParam.forEach(item=>{
                if(item){
                item=item.replace(/(^\s*)|(\s*$)/g, ""); //去掉前后空格
                let newItem=item;
                let firstChar=item.substr(0,1);
                if(firstChar!=='['){
                   let findindex=item.indexOf('.');
                   if(findindex==-1){
                        let containSpaceArray=item.split(' ');
                        if(containSpaceArray.length>=2){
                            newItem=item[1];
                        }
                   }else{
                        let containSpaceArray=item.split(' ');
                        if(containSpaceArray.length>=2){
                             newItem=containSpaceArray[1];
                             item=containSpaceArray[0];
                        }else{
