    ...
    addEvent("mouseup", function () {
        ...
        if (sDivId == "divCenter") {
            ...
            for (var c = 0; c < connections.length; c++) {
                ...
                $(connection.connector.path.outerHTML).attr("stroke", "white");
...
function MoveSelectDiv(event, ui, id) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        if (arr[i].parentNode.id !== document.getElementById(id).parentNode.id)
            continue;
...
function SelectClockwise(index) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        var points = instance.getEndpoints(arr[i]);
    }
...
function DeleteSelect() {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        //    instance.deleteEndpoint(points[j]);
        //}
