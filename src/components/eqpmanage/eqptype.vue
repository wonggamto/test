<template>
  <el-container style="height:100%;">
    <div class="myhead" style="padding-left:20px">
      <el-input style="width:150px;" v-model="TypeCode" clearable placeholder="设备类型编号"></el-input>
      <el-input style="width:150px;" v-model="TypeName" clearable placeholder="设备类型名称"></el-input>
    ...
    <el-main >
      <el-table   v-loading="loading"  element-loading-text="拼命加载中"  element-loading-spinner="el-icon-loading"
        ...
        <el-table-column prop="Eqp_TypeCode" label="设备类型编号" sortable min-width="150"></el-table-column>
        <el-table-column prop="Eqp_TypeName" label="设备类型名称" sortable min-width="120"></el-table-column>
        <el-table-column prop="Eqp_TypeStatus" label="设备类型状态"  :formatter="formatlogtype" sortable  min-width="120" ></el-table-column>
        <el-table-column prop="Eqp_Remark" label="设备类型备注" sortable min-width="180"></el-table-column>
      ...
      <el-form    ref="configForm">
          <el-form-item label="设备类别编号" :label-width="formLabelWidth">
          <el-input style="width:80%" :disabled="true"  v-model="dType.eqp_TypeCode" autocomplete="off" clearable></el-input>
        </el-form-item>
        <el-form-item label="设备类别名称" :label-width="formLabelWidth">
          <el-input style="width:80%" :disabled="isEnable" placeholder="请输入设备类别名称" v-model="dType.eqp_TypeName" autocomplete="off" clearable></el-input>
        </el-form-item>
        <el-form-item label="设备类别状态" :label-width="formLabelWidth">
          <el-select style="width:80%" :disabled="isEnable"    v-model="dType.eqp_TypeStatus" placeholder="设备类别状态">
        ...
        <el-form-item  label="设备类别备注"  :label-width="formLabelWidth">
              <el-input style="width:80%" :disabled="isEnable" placeholder="请输入描述" type="textarea" 
                        v-model="dType.eqp_Remark"  autocomplete="off" clearable></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    opendialog(flag, row) {
         ...
         if (flag === 1) {
            ...
            this.currentOpUrl = this.URL+"/basic/device/type/add";
            this.currentOpName="添加设备类型";
            this.dType.eqp_TypeCode ='d'+new Date().getTime();
        }
        else if(flag===2){
            this.loadDetail(row);
            this.operationFlag = 2;
            this.currentOpUrl =  this.URL+"/basic/device/type/update?id="+row.Eqp_TypeId;
            this.currentOpName="编辑设备类型";
        }
    },
    loadDetail(row) {
       if(row)
       {  
         this.$http.get( this.URL+'/basic/device/type/detail?id='+row.Eqp_TypeId)
         .then(resp=>{
    ...
    addObject() {
      ...
      if (!this.dType.eqp_TypeName) {
        this.$message({ message: "设备类型名称不能为空",
          type: "error",
          showClose: true
        });
        return false;
      ...
      if(dRow)
      {
         this.$confirm('此操作将永久该条数据, 是否继续?', '提示', {
          ...
          type: 'warning'
           ...
           this.$http.post(this.URL+"/basic/device/type/delete?id="+dRow.Eqp_TypeId,).then(resp=>{
               if(resp&&resp.data&&resp.data.result) {
                this.$message({message: "物料类型编码：" + dRow.Eqp_TypeCode + "删除成功！",type: "success",showClose: true});
                this.search();
                this.dialogFormVisible=false;
                } else {
    ...
    search(){
      ...
      let data = {
        ...
        "Eqp_TypeName":this.TypeName,
        "Eqp_TypeCode":this.TypeCode,
        "Eqp_TypeStatus":this.TypeStatus,
    ...
    clearForm(){
        ...
        this.id=0,
        this.dType.eqp_TypeCode='',
        this.dType.eqp_TypeName='',
        this.dType.eqp_Remark='' 
    },
    ...
    formatlogtype(row, column) {
      return this.getType(row.Eqp_TypeStatus);
    },
    getType(val) {
      let res = "";
      switch (val) {
