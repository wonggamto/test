  ...
  <div>
    <el-row type="flex" class="row-bg" style="height:35px;">
       <el-col :span="24">
         <div style="width:90%;">
              ...
              <el-button type="primary" @click="searchData()" icon="el-icon-edit"  align="left" >新增</el-button> 
              <el-button type="warning" @click="searchData()" icon="el-icon-edit"  align="left" >编辑</el-button> 
             <el-button type="danger" @click="searchData()" icon="el-icon-delete"  align="left" >删除</el-button>
          </div>
      </el-col>
    </el-row>
    ...
    <el-row class="row-bg">
      <el-col type="flex" :span="24" >
        <el-table class="table_report" highlight-current-row  @current-change="handleCurrentChangeTable"  v-loading="loading"  element-loading-text="拼命加载中"  element-loading-spinner="el-icon-loading"  tooltip-effect="dark" border ref="table" :data="mainData" stripe :height="`${tableHeight}px`">
          <template v-for='(item,index) in datacolumn'>
            <el-table-column sortable fixed :formatter="formatter" :width="item.width" v-if="item.isshow" :show-overflow-tooltip="true"   :key="index" align="center" :prop="item.prop" :label="item.label">
            </el-table-column> 
          </template>
        </el-table>
